    ...
    )
{
    ...
    err = netmgr_alloc_string(cmdName, &pCmd->pszCmd);
    bail_on_error(err);
...
error:
    if (pKeyVal)
    {
        netmgr_free(pKeyVal->pszKey);
        netmgr_free(pKeyVal->pszValue);
        netmgr_free(pKeyVal);
    }
    goto cleanup;
}
    ...
    )
{
    ...
    if(pCmd)
    {
        netmgr_free(pCmd->pszCmd);
        for (kv = pCmd->pCmdOpt; kv;)
        {
            kvnext = kv->pNext;
            netmgr_free(kv->pszKey);
            netmgr_free(kv->pszValue);
            netmgr_free(kv);
            kv = kvnext;
        }
        netmgr_free(pCmd);
