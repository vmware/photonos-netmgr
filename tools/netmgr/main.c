...
cmd_link_info(PNETMGR_CMD pCmd)
{
    ...
    size_t i = 0, count = 0;
    char *pszIfname = NULL, *pszMode = NULL, *pszState = NULL;
    char *pszMac_Addr = NULL, *pszMtu = NULL, *pszEnd = NULL;
    NET_LINK_INFO **ppNetLinkInfo = NULL;
    NET_LINK_MODE link_mode = LINK_MODE_UNKNOWN;
    NET_LINK_STATE link_state = LINK_STATE_UNKNOWN;
    ...
    switch(pCmd->op)
    {
        case OP_SET:
            ...
            if (pszMode!= NULL)
            {
                if (!strcmp(pszMode, "manual"))
                {
                    link_mode = LINK_MANUAL;
                }
                else if (!strcmp(pszMode, "auto"))
                {
                    link_mode = LINK_AUTO;
                }
                if (link_mode == LINK_MODE_UNKNOWN)
                {
                    err = EDOM;
                }
                bail_on_error(err);

                err = set_link_mode(pszIfname, link_mode);
                bail_on_error(err);
            }

            err = netmgrcli_find_cmdopt(pCmd, "state", &pszState);
            if (err == ENOENT)
            {
                err = 0;
            }
            ...
            if (pszState != NULL)
            {
                if (!strcmp(pszState, "up"))
                {
                    link_state = LINK_UP;
                }
                else if (!strcmp(pszState, "down"))
                {
                ...
                err = set_link_state(pszIfname, link_state);
                bail_on_error(err);
            }

            err = netmgrcli_find_cmdopt(pCmd, "mac_adrress", &pszMac_Addr);
            if (err == ENOENT)
            {
                err = 0;
            }
            ...
            if (pszMac_Addr != NULL)
            {
                err = set_link_mac_addr(pszIfname, pszMac_Addr);
                bail_on_error(err);
            }
        ...
        case OP_GET:
            ...
            err = get_link_info(pszIfname, &count, &ppNetLinkInfo);
            bail_on_error(err);
            fprintf(stdout, "Link Information\n");
            for (i = 0; i < count; i++)
            {
                fprintf(stdout, "Link #%zu\n", i+1);
                fprintf(stdout, "\tInterface = %s\n",ppNetLinkInfo[i]->pszInterfaceName);
                fprintf(stdout, "\tMTU = %lu\n",(unsigned long)ppNetLinkInfo[i]->mtu);
                fprintf(stdout, "\tMacAdrress = %s\n",ppNetLinkInfo[i]->pszMacAddress);
                fprintf(stdout, "\tState = %s\n",state_to_string(ppNetLinkInfo[i]->state));
                fprintf(stdout, "\tMode = %s\n",mode_to_string(ppNetLinkInfo[i]->mode));
            }
            break;
        default:
            err =  EINVAL;
...
cleanup:
    netmgr_free(pszIfname);
    netmgr_free(pszMac_Addr);
    netmgr_free(pszMode);
    netmgr_free(pszMtu);
    netmgr_free(pszState);
    if (ppNetLinkInfo){
        for (i = 0; i < count; i++)
        {
            if(ppNetLinkInfo[i])
            {
                netmgr_free(ppNetLinkInfo[i]->pszInterfaceName);
                netmgr_free(ppNetLinkInfo[i]->pszMacAddress);
            }
        }
    }
    netmgr_list_free(count, (void **)ppNetLinkInfo);
    return err;
error:
    goto cleanup;
}
