...
cmd_ip4_address(PNETMGR_CMD pCmd)
{
    ...
    switch (pCmd->op)
    {
        case OP_SET:
            ...
            {
                ip4Mode = IPV4ADDR_MODE_DHCP;
            }
            else if (!strcmp(pszMode, "static"))
            {
                ip4Mode = IPV4ADDR_MODE_STATIC;
            }
            else if (!strcmp(pszMode, "none"))
            {
                ip4Mode = IPV4ADDR_MODE_NONE;
            }
            else
            {
                err = EINVAL;
            ...
            err = set_ipv4addr_gateway(pszIfName, ip4Mode, pszIpAddr,
                                       pszGateway, 0);
            pszIpAddr = NULL;
            pszGateway = NULL;
            bail_on_error(err);
        ...
        case OP_GET:
            err = get_ipv4addr_gateway(pszIfName, &ip4Mode, &pszIpAddr,
                                       &pszGateway);
            bail_on_error(err);

            if (ip4Mode == IPV4ADDR_MODE_NONE)
            {
                fprintf(stdout, "IPv4 Address Mode: none\n");
            }
            else if (ip4Mode == IPV4ADDR_MODE_DHCP)
            {
...
cmd_ip6_address(PNETMGR_CMD pCmd)
{
    ...
    switch (pCmd->op)
    {
        ...
        case OP_SET:
            ...
            enableDhcp = is_dhcp_ipv6_enabled(pszIfName);
            if (enableDhcp < 0)
            {
                err = ENOMEM;
                bail_on_error(err);
            }
            enableAutoconf = is_autoconf_ipv6_enabled(pszIfName);
            if (enableAutoconf < 0)
            {
                err = ENOMEM;
                bail_on_error(err);
            ...
            if (pszDhcp != NULL)
            {
                enableDhcp = (!strcmp(pszDhcp, "1")) ? 1 : 0;
            }
            if (pszAutoconf != NULL)
            {
                enableAutoconf = (!strcmp(pszAutoconf, "1")) ? 1 : 0;
            }

            // TODO: Implement configuration of autoconf IPv6 enable / disable
            err = set_ipv6addr_mode(pszIfName, enableDhcp, enableAutoconf);
            bail_on_error(err);
            break;
