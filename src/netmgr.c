...
)
{
    ...
    {
        err = get_key_value(cfgFileName, szSectionName, KEY_DOMAIN,
                            &szCurrentDnsDomains);
        if (err != ENOENT)
        {
            bail_on_error(err);
    ...
    if (count == 0)
    {
        err = set_key_value(cfgFileName, szSectionName, KEY_DOMAIN, NULL, 0);
    }
    else
    {
        err = set_key_value(cfgFileName, szSectionName, KEY_DOMAIN,
                            szDnsDomainsValue, 0);
    }
    bail_on_error(err);
...
)
{
    ...
    /* Parse pszDnsServersValue */
    err = get_key_value(cfgFileName, szSectionName, KEY_DOMAIN, &pszDnsDomainsValue);
    if (err == ENOENT)
    {
        err = 0;
        goto error;
