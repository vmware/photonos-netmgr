...
const char *nm_link_state_to_string(NET_LINK_STATE state) {
...
const char *nm_link_mode_to_string(NET_LINK_MODE mode) {
...
const char *nm_ip_addr_type_to_string(NET_ADDR_TYPE addrType) {
        switch (addrType){
                case STATIC_IPV4:
                        return "IPv4 static";
                case STATIC_IPV6:
...
const char *nm_get_error_info(uint32_t nmErrCode) {
        switch (nmErrCode) {
...
static uint32_t nm_alloc_conf_filename(char **file, const char *path, const char *file_name) {
        ...
        len = strlen(path) + strlen (file_name) + 1;
        err = netmgr_alloc(len, (void **)&pszFilename);
        bail_on_error(err);

        sprintf(pszFilename, "%s%s", path, file_name);
...
static uint32_t nm_get_resolved_conf_filename(char **file) {
        return nm_alloc_conf_filename(file, SYSTEMD_PATH, "resolved.conf");
}

static uint32_t nm_get_networkd_conf_filename(char **file){
        return nm_alloc_conf_filename(file, SYSTEMD_PATH, "networkd.conf");
}

static uint32_t nm_read_conf_file(const char *file, char **file_buf) {
       _cleanup_(freep) char *pszFileBuf = NULL;
       _cleanup_(fclosep) FILE *fp = NULL;
        uint32_t err = 0;
        long len;

        if (IS_NULL_OR_EMPTY(file) || !file_buf) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
...
static uint32_t nm_regex_match_ifname(const char *ifname, const char *match) {
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !IS_VALID_INTERFACE_NAME(match)) {
                err = NM_ERR_INVALID_INTERFACE;
                bail_on_error(err);
        }

        ifNameLen = strlen(ifname);
        patternLen = strlen(match);
        for (p = strchr(match, '*'), n = 0; p; p = strchr(++p, '*'), n++);

        err = netmgr_alloc(patternLen + n + 1, (void **)&pszPattern);
        bail_on_error(err);

        for (p = match, q = pszPattern; *p; p++, q++) {
                if (*p == '*')
                        *q++ = '.';
...
static uint32_t nm_get_network_conf_filename_match(const char *ifname, char **file, size_t *pMatchLen) {
        char *p, *pszFileName = NULL, *match = NULL, *pszCfgFileName = NULL;
        size_t matchLen = 0;
        struct dirent *hFile;
        DIR *dirFile = NULL;
        uint32_t err = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || !file) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
                        ...
                        if ((((p = strstr(hFile->d_name, ".network")) != NULL) &&
                            ...
                            (((p = strstr(hFile->d_name, ".network.manual")) != NULL) &&
                             (strlen(p) == strlen(".network.manual")))) {
                                err = nm_alloc_conf_filename(&pszFileName, SYSTEMD_NET_PATH, hFile->d_name);
                                bail_on_error(err);

                                err = nm_get_key_value(pszFileName, SECTION_MATCH, KEY_NAME, &match);
                                if ((err == NM_ERR_VALUE_NOT_FOUND) || (err == NM_ERR_BAD_CONFIG_FILE))
                                        /* Ignore cfg file with invalid/missing Match section */
                                        err = 0;

                                bail_on_error(err);

                                if (match && !nm_regex_match_ifname(ifname, match)) {
...
static uint32_t nm_get_network_conf_filename(const char *ifname, char **file) {
        return nm_get_network_conf_filename_match(ifname, file, NULL);
}

static uint32_t nm_get_network_conf_filename_for_update(const char *ifname, char **ret) {
        char fName[IFNAMSIZ+strlen(SYSTEMD_NET_PATH)+strlen("00-.network")+1];
        char *file = NULL, *pszNewFilename = NULL, *pszCmd = NULL;
        size_t ifNameLen, matchLen = 0;
        uint32_t err = 0;

        // TODO: IS VALID_INTERFACE_NAME needs to check if device actually exists
        if (!IS_VALID_INTERFACE_NAME(ifname) || !ret) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        err = nm_get_network_conf_filename_match(ifname, &file, &matchLen);
        ...
        ifNameLen = strlen(ifname);
        if ((file == NULL) || (matchLen < ifNameLen)) {
                sprintf(fName, "%s00-%s.network", SYSTEMD_NET_PATH, ifname);
                ...
                err = nm_set_key_value(pszNewFilename, SECTION_MATCH, KEY_NAME, ifname, F_CREATE_CFG_FILE);
                bail_on_error(err);

        } else {
                err = netmgr_alloc_string(file, &pszNewFilename);
                bail_on_error(err);
        }
...
static uint32_t nm_get_network_auto_conf_filename(const char *ifname, char **file) {
        char *conf_file = NULL, *pManual = NULL;
        uint32_t err = 0;
        ...
        err = nm_get_network_conf_filename_for_update(ifname, &conf_file);
        bail_on_error(err);

        if ((pManual = strstr(conf_file, ".manual")) != NULL)
                *pManual = '\0';
 ...
 error:
        ...
        netmgr_free(conf_file);
        goto cleanup;
}

static uint32_t nm_get_network_manual_conf_filename(const char *ifname, char **file) {
        char *conf_file = NULL, *manual = NULL;
        uint32_t err = 0;
        ...
        if ((manual = strstr(conf_file, ".manual")) == NULL) {
                err = nm_alloc_conf_filename(file, conf_file, ".manual");
                bail_on_error(err);
                netmgr_free(conf_file);
        } else
                *file = conf_file;
...
static uint32_t nm_string_to_line_array(const char *pszStrBuf, size_t *pLineCount, char ***pppszLineBuf) {
...
uint32_t nm_touch_network_conf_file(const char *ifname, char **file) {
        _cleanup_(freep) char *file_name = NULL;
        ...
        err = nm_get_network_conf_filename_for_update(ifname, &file_name);
        bail_on_error(err);

        if (file != NULL)
                *file = file_name;
        else
                netmgr_free(file_name);
...
static uint32_t nm_update_link_state(const char *ifname, NET_LINK_STATE linkState);

static uint32_t nm_update_mac_address(const char *ifname, const char *mac_address) {
        ...
        nlMsg.linkInfo.ifi_index = if_nametoindex(ifname);
        addrLen = sscanf(mac_address, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
        ...
        err = netlink_add_attr(&nlMsg.nlHdr, IFLA_ADDRESS, &mac_address, ETH_ALEN);
        bail_on_error(err);
...
uint32_t nm_set_link_mac_addr(const char *ifname, const char *mac) {
      char *file_name = NULL, *pszOldMacAddress = NULL;
      ...
      int lockId;
        ...
        err = nm_get_link_mac_addr(ifname, &pszOldMacAddress);
        bail_on_error(err);

        if (strlen(mac) > 0) {
                err = nm_update_mac_address(ifname, mac);
                bail_on_error(err);
        }

        err = nm_set_key_value(file_name, SECTION_LINK, KEY_MAC_ADDRESS, strlen(mac) ? mac : NULL, 0);
        if (err) {
                err1 = nm_update_mac_address(ifname, pszOldMacAddress);
                bail_on_error(err1);
        }
...
uint32_t nm_get_link_mac_addr(const char *ifname, char **pmac) {
        _cleanup_(freep) char *pszLine = NULL, *path = NULL;
        _cleanup_(fclosep) FILE *fp = NULL;
        uint32_t err = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || !pmac) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        err = netmgr_alloc_string_printf(&path, "/sys/class/net/%s/address", ifname);
        bail_on_error(err);

        err = nm_read_one_line(path, &pszLine);
        bail_on_error(err);

        err = netmgr_alloc_string(pszLine, pmac);
        bail_on_error(err);
...
static uint32_t nm_update_link_mode(const char *ifname, NET_LINK_MODE linkMode) {
        char *pszAutoCfgFileName = NULL, *pszManualCfgFileName = NULL;
        uint32_t err = 0, sdVersion = 0;
        char *pszCurrentCfgFileName = NULL;

        if (!IS_VALID_INTERFACE_NAME(ifname) || (linkMode >= LINK_MODE_UNKNOWN)) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = nm_get_network_conf_filename_for_update(ifname, &pszCurrentCfgFileName);
        bail_on_error(err);

        if (sdVersion >= 233) {
                switch (linkMode) {
                case LINK_MANUAL:
                        err = nm_set_key_value(pszCurrentCfgFileName, SECTION_LINK, KEY_UNMANAGED, "yes", 0);
                        break;
                case LINK_AUTO:
                        err = nm_set_key_value(pszCurrentCfgFileName, SECTION_LINK, KEY_UNMANAGED, "no", 0);
                        break;
                default:
                        err = NM_ERR_INVALID_PARAMETER;
                        break;
                ...
                err = nm_get_network_auto_conf_filename(ifname, &pszAutoCfgFileName);
                bail_on_error(err);

                err = nm_get_network_manual_conf_filename(ifname, &pszManualCfgFileName);
                bail_on_error(err);
...
uint32_t nm_set_link_mode(const char *ifname, NET_LINK_MODE linkMode) {
        uint32_t err = 0;
        int lockId;
        ...
        err = nm_update_link_mode(ifname, linkMode);
        bail_on_error(err);
...
uint32_t nm_get_link_mode(const char *ifname, NET_LINK_MODE *pLinkMode) {
        char *file_name = NULL, *pszUnmanaged = NULL;;
        uint32_t err = 0, sdVersion = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || !pLinkMode) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = nm_get_network_conf_filename(ifname, &file_name);
        bail_on_error(err);

        if (sdVersion >= 233) {
                err = nm_get_key_value(file_name, SECTION_LINK, KEY_UNMANAGED, &pszUnmanaged);
                        if (err == NM_ERR_VALUE_NOT_FOUND) {
                                err = 0;
                                *pLinkMode = LINK_AUTO;
                        } else if (err == 0) {
                ...
                if (strstr(file_name, ".manual"))
                        *pLinkMode = LINK_MANUAL;
                else if (strstr(file_name, ".network"))
                        *pLinkMode = LINK_AUTO;
                else
                        *pLinkMode = LINK_MODE_UNKNOWN;
        }
...
static uint32_t nm_update_link_mtu(const char *ifname, uint32_t mtu) {
      _cleanup_(closep) int sockFd = -1;
        uint32_t err = 0;
        ...
        nlMsgLink.linkInfo.ifi_index = if_nametoindex(ifname);
...
uint32_t nm_set_link_mtu(const char *ifname, uint32_t mtu) {
        ...
        err = nm_get_link_mtu(ifname, &oldMtu);
        bail_on_error(err);

        if (mtu == 0)
                mtu = DEFAULT_MTU_VALUE;

        err = nm_update_link_mtu(ifname, mtu);
        bail_on_error(err);

        sprintf(szValue, "%u", mtu);

        err = nm_set_key_value(file_name, SECTION_LINK, KEY_MTU, szValue, 0);
        if (err) {
                err1 = nm_update_link_mtu(ifname, oldMtu);
                bail_on_error(err1);
        }
...
uint32_t nm_get_link_mtu(const char *ifname, uint32_t *pMtu) {
        _cleanup_(freep) char *pszLine = NULL, *path = NULL;
        _cleanup_(fclosep) FILE *fp = NULL;
        uint32_t err = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || !pMtu) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        err = netmgr_alloc_string_printf(&path, "/sys/class/net/%s/mtu", ifname);
        bail_on_error(err);
...
static uint32_t nm_update_link_state(const char *ifname, NET_LINK_STATE linkState) {
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || (linkState >= LINK_STATE_UNKNOWN)) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        err = nm_get_link_state(ifname, &linkStateCurrent);
        bail_on_error(err);
...
uint32_t nm_set_link_state(const char *ifname, NET_LINK_STATE linkState) {
        uint32_t err = 0;
        int lockId;
        ...
        err = nm_update_link_state(ifname, linkState);
        bail_on_error(err);
...
uint32_t nm_get_link_state(const char *ifname, NET_LINK_STATE *pLinkState) {
        _cleanup_ (closep) int sockFd = -1;
        struct ifreq ifr = {};
        uint32_t err = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || !pLinkState) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
...
static uint32_t nm_do_arping(const char *ifname, const char *pszCommandOptions, const char *pszDestIPv4Addr) {
        char *pszArpingCmd = NULL;
        uint32_t err = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || IS_NULL_OR_EMPTY(pszCommandOptions) ||
            IS_NULL_OR_EMPTY(pszDestIPv4Addr)) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        err = netmgr_alloc_string_printf(&pszArpingCmd, "%s %s -I %s %s", ARPING_COMMAND,
                                         pszCommandOptions, ifname, pszDestIPv4Addr);
        bail_on_error(err);
...
#if 0
static uint32_t nm_do_ndsend(const char *ifname, const char *pszCommandOptions, const char *pszDestIPv6Addr){
        //TODO: Implement
        return 00;
}
#endif

static uint32_t nm_get_ip_dhcp_mode(const char *ifname, uint32_t *pDhcpModeFlags);

static uint32_t nm_get_static_ip_addr(
                const char *ifname,
                uint32_t addrTypes,
                size_t *pCount,
                char ***pppszIpAddrList);

static uint32_t nm_get_interface_ipaddr(
                const char *ifname,
                NET_ADDR_TYPE addrType,
                size_t *pCount,
                char ***pppszIpAddress);

uint32_t nm_ifup(const char *ifname) {
        ...
        err = nm_get_link_state(ifname, &linkState);
        bail_on_error(err);

        err = nm_get_interface_ipaddr(ifname, STATIC_IPV4, &ip4Count,&ppszIpv4AddrList);
        if (err == NM_ERR_VALUE_NOT_FOUND)
                err = 0;
        ...
        if (!ip4Count) {
                err = nm_get_interface_ipaddr(ifname, STATIC_IPV6, &ip6Count, &ppszIpv6AddrList);
                if (err == NM_ERR_VALUE_NOT_FOUND)
                        err = 0;
                ...
                err = nm_get_static_ip_addr(ifname, STATIC_IPV4, &staticIp4Count, &ppszStaticIpv4AddrList);
                if (err == NM_ERR_VALUE_NOT_FOUND)
                        err = 0;

                bail_on_error(err);

                err = nm_get_static_ip_addr(ifname, STATIC_IPV6, &staticIp6Count, &ppszStaticIpv6AddrList);
                if (err == NM_ERR_VALUE_NOT_FOUND)
                        err = 0;

                bail_on_error(err);

                err = flush_interface_ipaddr(ifname);
                bail_on_error(err);

                err = nm_update_link_state(ifname, LINK_UP);
                bail_on_error(err);

                if (staticIp4Count && ip4Count && (strcmp(ppszStaticIpv4AddrList[0], ppszIpv4AddrList[0]) != 0)) {
                        err = NM_ERR_BAD_CONFIG_FILE;
                        bail_on_error(err);
                }

                if (staticIp4Count && !ip4Count) {
                        retVal = sscanf(ppszStaticIpv4AddrList[0], "%[^/]/%hhu", ipAddr, &prefix);
                        if ((retVal != 1) && (retVal != 2)) {
                                err = NM_ERR_BAD_CONFIG_FILE;
                                bail_on_error(err);
                        }

                        err = nm_do_arping(ifname, ARPING_DUP_ADDR_CHECK_CMDOPT, ipAddr);
                        bail_on_error(err);
                }

                err = nm_get_link_mode(ifname, &linkMode);
                bail_on_error(err);

                if (linkMode == LINK_MANUAL) {
                        err = nm_update_link_mode(ifname, LINK_AUTO);
                        bail_on_error(err);
                }

                err = nm_restart_network_service();
                if (linkMode == LINK_MANUAL) {
                        err1 = nm_update_link_mode(ifname, LINK_MANUAL);
                        bail_on_error(err1);
                }
                bail_on_error(err);

                err = nm_get_ip_dhcp_mode(ifname, &dhcpModeFlags);
                bail_on_error(err);

                if (staticIp4Count || staticIp6Count ||
                    TEST_FLAG(dhcpModeFlags, fDHCP_IPV4) ||
                    TEST_FLAG(dhcpModeFlags, fDHCP_IPV6)) {
                        err = nm_wait_for_ip(ifname, DEFAULT_WAIT_FOR_IP_TIMEOUT, NET_ADDR_IPV4 | NET_ADDR_IPV6);
                        bail_on_error(err);
                }
        }

        if (staticIp4Count && !ip4Count) {
                err = nm_do_arping(ifname, ARPING_UPDATE_NEIGHBOR_CMDOPT, ipAddr);
                bail_on_error(err);
        }
...
uint32_t nm_ifdown(const char *ifname) {
        uint32_t err = 0;
        ...
        err = nm_set_link_state(ifname, LINK_DOWN);
        bail_on_error(err);
...
static void nm_free_interface_list(PNET_INTERFACE pInterfaceList) {
...
static uint32_t nm_enumerate_systemd_interfaces(PNET_INTERFACE *ppInterfaces) {
...
static uint32_t nm_get_interface_info(const char *ifname, NET_LINK_INFO **ppLinkInfo) {
        NET_LINK_STATE linkState = LINK_STATE_UNKNOWN;
        NET_LINK_MODE linkMode = LINK_MODE_UNKNOWN;
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !ppLinkInfo){
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = nm_get_link_mac_addr(ifname, &mac);
        bail_on_error(err);

        err = nm_get_link_mtu(ifname, &mtu);
        bail_on_error(err);
        ...
        err = netmgr_alloc_string(ifname, &pLinkInfo->ifname);
        bail_on_error(err);
...
uint32_t nm_get_link_info(const char *ifname, NET_LINK_INFO **ppLinkInfo) {
        ...
        if (IS_NULL_OR_EMPTY(ifname)) {
                ...
                err = nm_get_interface_info(ifname, &pLinkInfo);
                bail_on_error(err);
        }
...
void nm_free_link_info(NET_LINK_INFO *pNetLinkInfo) {
        NET_LINK_INFO *pCurrent = NULL;

        while (pNetLinkInfo) {
                        pCurrent = pNetLinkInfo;
                        pNetLinkInfo = pNetLinkInfo->pNext;
                        netmgr_free(pCurrent->mac);
                        netmgr_free(pCurrent->ifname);
                        netmgr_free(pCurrent);
        }
}
...
static uint32_t nm_get_static_ip_addr(
        ...
        char *file_name = NULL, ipAddr[INET6_ADDRSTRLEN];
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pCount || !pppszIpAddrList) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = ini_cfg_read(file_name, &pConfig);
        bail_on_error(err);
                ...
                do {
                        ...
                        if ((TEST_FLAG(addrTypes, STATIC_IPV4) && is_ipv4_addr(ipAddr)) ||
                            (TEST_FLAG(addrTypes, STATIC_IPV6) && is_ipv6_addr(ipAddr))) {
                                err = netmgr_alloc_string(pNextKeyValue->pszValue, &(ppszIpAddrList[i++]));
                                bail_on_error(err);
                        }
...
static uint32_t nm_get_interface_ipaddr(
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pCount || !pppszIpAddress) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        if (TEST_FLAG(addrType, STATIC_IPV6) || TEST_FLAG(addrType, DHCP_IPV6) || TEST_FLAG(addrType, AUTO_IPV6))
                ip6Type = 1;
        ...
        for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
                ...
                if (((af == AF_INET) && ip4Type) || ((af == AF_INET6) && ip6Type))
                        count++;
        }
        ...
        for (ifa = ifaddr; count && (ifa != NULL); ifa = ifa->ifa_next) {
                        ...
                        if (((af == AF_INET) && ip4Type) || ((af == AF_INET6) && ip6Type)) {
                                ...
                                case AF_INET:
                                        ...
                                        pszIpAddr = inet_ntop(af, (void *)&sockIpv4->sin_addr, szIpAddr, INET_ADDRSTRLEN);
                                        break;
                                case AF_INET6:
                                        sockIpv6 = (struct sockaddr_in6 *)ifa->ifa_addr;
                                        pszIpAddr = inet_ntop(af, (void *)&sockIpv6->sin6_addr, szIpAddr, INET6_ADDRSTRLEN);
                                        break;
                                }
                                ...
                                err = netmgr_alloc_string_printf(&ppszIpAddrList[i++], "%s/%hhu", szIpAddr, prefix);
                                bail_on_error(err);
                        }
        }
...
static uint32_t nm_get_ip_default_gateway(
                const char *ifname,
                NET_ADDR_TYPE addrType,
                char **ppszGateway) {
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !ppszGateway) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        for (; NLMSG_OK(nlMsgRoute, readSize); nlMsgRoute = NLMSG_NEXT(nlMsgRoute, readSize)) {
                ...
                if ((addrType == STATIC_IPV4) || (addrType == DHCP_IPV4)) {
                        if ((dst4.s_addr == 0) && !strcmp(szIfName, ifname)) {
...
static uint32_t nm_set_sysctl_persistent_value(const char *pszSysctlKey, const char *pszValue ) {
...
static uint32_t nm_get_ipv6_enable(const char *ifname, uint32_t *pEnabled);

static uint32_t nm_set_ip_dhcp_mode(const char *ifname, uint32_t dhcpModeFlags) {
        ...
        err = nm_get_network_conf_filename_for_update(ifname,
                                                      &file_name);
        bail_on_error(err);
        ...
        err = nm_set_key_value(file_name, SECTION_NETWORK, KEY_DHCP, szDhcpValue, 0);
        bail_on_error(err);
...
static uint32_t nm_get_ip_dhcp_mode(const char *ifname, uint32_t *pDhcpModeFlags) {
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pDhcpModeFlags) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = nm_get_key_value(file_name, SECTION_NETWORK, KEY_DHCP, &pszDhcpValue);
        ...
        err = nm_get_ipv6_enable(ifname, &v6enabled);
...
static uint32_t nm_set_static_ip_gateway(const char *ifname, const char *pszIpGwAddr);
static uint32_t nm_delete_static_ip_gateway(const char *ifname, uint32_t addrTypes);
static uint32_t nm_get_static_ip_gateway(
                const char *ifname,
                uint32_t addrTypes,
                size_t *pCount,
                char ***pppszGwAddrList);
static uint32_t nm_set_static_ipv4_addr(
                const char *ifname,
                const char *pszIPv4Addr,
                uint8_t prefix);

static uint32_t nm_delete_static_ipv4_addr(const char *ifname);
static uint32_t nm_set_static_ip_gateway(const char *ifname, const char *pszIpGwAddr) {
        uint32_t err = 0, addrType;
        char *file_name = NULL;;

        if (!IS_VALID_INTERFACE_NAME(ifname) || IS_NULL_OR_EMPTY(pszIpGwAddr)) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = nm_delete_static_ip_gateway(ifname, addrType);
        bail_on_error(err);

        err = nm_add_key_value(file_name, SECTION_NETWORK, KEY_GATEWAY,
                               pszIpGwAddr, 0);
        bail_on_error(err);
...
static uint32_t nm_delete_static_ip_gateway(const char *ifname, uint32_t addrTypes) {
        char *file_name = NULL, **ppszGwAddrList = NULL;
        uint32_t err = 0;
        size_t i, count = 0;
        ...
        err = nm_get_static_ip_gateway(ifname, addrTypes, &count, &ppszGwAddrList);
        bail_on_error(err);

        for (i = 0; i < count; i++) {
                        err = nm_delete_key_value(file_name, SECTION_NETWORK, KEY_GATEWAY, ppszGwAddrList[i], 0);
                        bail_on_error(err);
        }
...
static uint32_t nm_get_static_ip_gateway(
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pCount || !pppszGwAddrList) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        do {
                ...
                if (TEST_FLAG(addrTypes, STATIC_IPV4) && is_ipv4_addr(pNextKeyValue->pszValue))
                        nCount++;
                else if (TEST_FLAG(addrTypes, STATIC_IPV6) && is_ipv6_addr(pNextKeyValue->pszValue))
                                        nCount++;
        ...
        if (nCount > 0) {
                ...
                do {
                        ...
                        sscanf(pNextKeyValue->pszValue, "%[^/]/%u", ipAddr, &prefix);
                        if ((TEST_FLAG(addrTypes, STATIC_IPV4) && is_ipv4_addr(pNextKeyValue->pszValue)) ||
                            (TEST_FLAG(addrTypes, STATIC_IPV6) && is_ipv6_addr(pNextKeyValue->pszValue))) {
                                err = netmgr_alloc_string(pNextKeyValue->pszValue, &(ppszGwAddrList[i++]));
                                bail_on_error(err);
                        }
                        pKeyValue = pNextKeyValue;
                } while (pNextKeyValue != NULL);
...
static uint32_t nm_set_static_ipv4_addr(
        ...
        char *file_name = NULL, szIpAddr[MAX_LINE];
        uint32_t err = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || IS_NULL_OR_EMPTY(pszIPv4Addr)) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = nm_delete_static_ipv4_addr(ifname);
        if (err == NM_ERR_VALUE_NOT_FOUND)
                err = 0;

        bail_on_error(err);

        err = nm_add_key_value(file_name, SECTION_NETWORK, KEY_ADDRESS, szIpAddr, 0);
        bail_on_error(err);
...
static uint32_t nm_delete_static_ipv4_addr(const char *ifname) {
        char *file_name = NULL, **ppszIpAddrList = NULL;
        uint32_t err = 0;
        size_t count = 0;
        ...
        err = nm_get_static_ip_addr(ifname, STATIC_IPV4, &count, &ppszIpAddrList);
        bail_on_error(err);
        if (count > 1) {
                err = NM_ERR_BAD_CONFIG_FILE;
                bail_on_error(err);
        ...
        if (count) {
                err = nm_delete_key_value(file_name, SECTION_NETWORK, KEY_ADDRESS, ppszIpAddrList[0], 0);
                bail_on_error(err);
        }
...
uint32_t nm_set_ipv4_addr_gateway(
                const char *ifname,
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || (mode >= IPV4_ADDR_MODE_MAX)) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        if ((mode == IPV4_ADDR_MODE_STATIC) && (IS_NULL_OR_EMPTY(pszIPv4AddrPrefix) ||
        ...
        err = nm_get_ip_dhcp_mode(ifname, &currModeFlags);
        bail_on_error(err);
        ...
        err = nm_delete_static_ip_gateway(ifname, STATIC_IPV4);
        if (err == NM_ERR_VALUE_NOT_FOUND)
                err = 0;
        ...
        switch (mode) {
                case IPV4_ADDR_MODE_STATIC:
                        err = nm_set_static_ipv4_addr(ifname, szIpAddr, prefix);
                        bail_on_error(err);
                        if (!IS_NULL_OR_EMPTY(pszIPv4Gateway)) {
                                        err = nm_set_static_ip_gateway(ifname, pszIPv4Gateway);
                                        bail_on_error(err);
                        }
                        /* fall-thru */
        ...
        err = nm_set_ip_dhcp_mode(ifname, currModeFlags);
        bail_on_error(err);
...
uint32_t nm_get_ipv4_addr_gateway(
                const char *ifname,
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !(pMode || ppszIPv4AddrPrefix || ppszIPv4Gateway)) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        err = nm_get_ip_dhcp_mode(ifname, &modeFlags);
        bail_on_error(err);

        if (TEST_FLAG(modeFlags, fDHCP_IPV4)) {
                ip4Mode = IPV4_ADDR_MODE_DHCP;

                err = nm_get_interface_ipaddr(ifname, DHCP_IPV4, &ipCount, &ppszIpAddrList);
                        if (err == NM_ERR_VALUE_NOT_FOUND)
                                err = 0;
                        ...
                        err = nm_get_ip_default_gateway(ifname, DHCP_IPV4, &pszIPv4Gateway);
                        if (err == NM_ERR_VALUE_NOT_FOUND)
                                        err = 0;
                        bail_on_error(err);
        } else {
                /* Get IP addresss from interface via API. If that fails, read file */
                err = nm_get_interface_ipaddr(ifname, STATIC_IPV4, &ipCount, &ppszIpAddrList);
                if (err == NM_ERR_VALUE_NOT_FOUND)
                        err = 0;
                ...
                if (ipCount == 0) {
                        err = nm_get_static_ip_addr(ifname, STATIC_IPV4, &ipCount, &ppszIpAddrList);
                                        if (err == NM_ERR_VALUE_NOT_FOUND)
                                                err = 0;
                                        bail_on_error(err);
                }
                ...
                if (ppszIpAddrList != NULL) {
                        ...
                        err = nm_get_ip_default_gateway(ifname, STATIC_IPV4, &pszIPv4Gateway);
                        if (err == NM_ERR_VALUE_NOT_FOUND)
                                err = 0;

                        bail_on_error(err);
                        if (pszIPv4Gateway == NULL) {
                                err = nm_get_static_ip_gateway(ifname, STATIC_IPV4, &gwCount, &ppszGwAddrList);
                                bail_on_error(err);
...
static uint32_t nm_set_ipv6_enable(const char *ifname, uint32_t enabled) {
        uint32_t err = 0;
...
static uint32_t nm_get_ipv6_enable(const char *ifname, uint32_t *pEnabled) {
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pEnabled) {
                        err = NM_ERR_INVALID_PARAMETER;
                        bail_on_error(err);
        }
...
uint32_t nm_add_static_ipv6_addr(const char *ifname, const char *pszIPv6AddrPrefix) {
        int n = 0;
        uint32_t err = 0, v6enabled = 0;
        uint8_t prefix = 0;
        char *file_name = NULL, szIpAddr[INET6_ADDRSTRLEN];
        int lockId;

        err = nm_acquire_write_lock(0, &lockId);
        bail_on_error(err);

        if (!IS_VALID_INTERFACE_NAME(ifname) || IS_NULL_OR_EMPTY(pszIPv6AddrPrefix) ||
        ...
        err = nm_add_key_value(file_name, SECTION_NETWORK, KEY_ADDRESS, pszIPv6AddrPrefix, 0);
        bail_on_error(err);
        ...
        if (!v6enabled) {
                err = nm_set_ipv6_enable(ifname, 1);
                bail_on_error(err);
        }
...
uint32_t nm_delete_static_ipv6_addr(const char *ifname, const char *pszIPv6AddrPrefix) {
        uint32_t err = 0, autov6, dhcpv6, v6enabled;
        uint8_t prefix = 0;
        char *file_name = NULL, szIpAddr[INET6_ADDRSTRLEN], **ppszIp = NULL;
        int n, lockId;
        size_t c = 0;
        ...
        err = nm_delete_key_value(file_name, SECTION_NETWORK, KEY_ADDRESS, pszIPv6AddrPrefix, 0);
        bail_on_error(err);

        /* Disable IPv6 if no autov6, no dhcpv6, and no staticv6 configured */
        err = nm_get_ipv6_addr_mode(ifname, &autov6, &dhcpv6);
        bail_on_error(err);

        if (!autov6 && !dhcpv6) {
                        err = nm_get_static_ip_addr(ifname, STATIC_IPV6, &c, &ppszIp);
                        if (err == NM_ERR_VALUE_NOT_FOUND)
                                        err = 0;
                        bail_on_error(err);
                        ...
                        if (c == 0) {
                                err = nm_get_ipv6_enable(ifname, &v6enabled);
                                        ...
                                        if (v6enabled) {
                                                        err = nm_set_ipv6_enable(ifname, 0);
                                                        bail_on_error(err);
                                                }
                                }
                }
...
uint32_t nm_set_ipv6_addr_mode(const char *ifname, uint32_t enableDhcp, uint32_t enableAutoconf) {
        ...
        err = nm_set_ip_dhcp_mode(ifname, modeFlags);
        bail_on_error(err);
...
uint32_t nm_get_ipv6_addr_mode(const char *ifname, uint32_t *pDhcpEnabled, uint32_t *pAutoconfEnabled) {
...
static uint32_t nm_get_ip_addr_type(const char *ifname, const char *pszIpAddr, NET_ADDR_TYPE *pAddrType) {
        char ipAddr[INET6_ADDRSTRLEN], **ppszIpAddrList = NULL, *pszMacAddr = NULL;
        uint32_t err = 0, prefix;
        size_t i, count = 0;
        NET_ADDR_TYPE addrType = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || IS_NULL_OR_EMPTY(pszIpAddr) || !pAddrType ||
        ...
        if (is_ipv6_addr(ipAddr)) {
                ...
                err = nm_get_static_ip_addr(ifname, STATIC_IPV6, &count, &ppszIpAddrList);
                if (err == NM_ERR_VALUE_NOT_FOUND)
                        err = 0;
                ...
                if (addrType == 0) {
                        ...
                        err = nm_get_link_mac_addr(ifname, &pszMacAddr);
                        bail_on_error(err);
...
uint32_t nm_get_ip_addr(
        ...
        //TODO: If ifname is NULL, enumerate all IPs
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pCount || !pppIpAddrList || !addrTypes) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        if (TEST_FLAG(modeFlags, fDHCP_IPV4) && TEST_FLAG(addrTypes, DHCP_IPV4)) {
                        err = nm_get_interface_ipaddr(ifname, DHCP_IPV4, &ip4Count, &ppszIp4AddrList);
                        if (err == NM_ERR_VALUE_NOT_FOUND)
                                        err = 0;

                        bail_on_error(err);
        } else if (TEST_FLAG(addrTypes, STATIC_IPV4)) {
                        err = nm_get_static_ip_addr(ifname, STATIC_IPV4, &ip4Count, &ppszIp4AddrList);
                        if (err == NM_ERR_VALUE_NOT_FOUND)
                                err = 0;
        }
        ...
        if (TEST_FLAG(addrTypes, DHCP_IPV6) || TEST_FLAG(addrTypes, AUTO_IPV6) ||
            TEST_FLAG(addrTypes, STATIC_IPV6) || TEST_FLAG(addrTypes, LINK_LOCAL_IPV6)) {
                err = nm_get_interface_ipaddr(ifname,
                                              NET_ADDR_IPV6 | LINK_LOCAL_IPV6,
                                              &ip6Count,
                                              &ppszIp6AddrList);

                if (err == NM_ERR_VALUE_NOT_FOUND) {
                                err = nm_get_static_ip_addr(ifname,
                                                            STATIC_IPV6,
                                                            &ip6Count,
                                                            &ppszIp6AddrList);
                                if (err == NM_ERR_VALUE_NOT_FOUND)
        ...
        for (i = 0; i < ip4Count; i++) {
                        ...
                        bail_on_error(err);
                        err = netmgr_alloc_string(ifname,
                                                  &ppIpAddrList[i]->ifname);
                        bail_on_error(err);
                        err = netmgr_alloc_string(ppszIp4AddrList[i],
                                                  &ppIpAddrList[i]->pszIPAddrPrefix);
                        bail_on_error(err);
                        ppIpAddrList[i]->type = TEST_FLAG(modeFlags, fDHCP_IPV4) ? DHCP_IPV4 : STATIC_IPV4;
                }

        for (j = 0; j < ip6Count; j++) {
                        err = nm_get_ip_addr_type(ifname, ppszIp6AddrList[j], &addrType);
                        bail_on_error(err);
                        ...
                        err = netmgr_alloc(sizeof(NET_IP_ADDR), (void **)&ppIpAddrList[i]);
                        bail_on_error(err);
                        err = netmgr_alloc_string(ifname, &ppIpAddrList[i]->ifname);
                        bail_on_error(err);
                        err = netmgr_alloc_string(ppszIp6AddrList[j], &ppIpAddrList[i]->pszIPAddrPrefix);
                        bail_on_error(err);
...
uint32_t nm_set_ipv6_gateway(const char *ifname, const char *pszIPv6Gateway) {
        uint32_t err = 0;
        int lockId;
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || (!IS_NULL_OR_EMPTY(pszIPv6Gateway) && !is_ipv6_addr(pszIPv6Gateway))) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        if (pszIPv6Gateway) {
                err = nm_set_static_ip_gateway(ifname, pszIPv6Gateway);
                bail_on_error(err);
        } else {
                err = nm_delete_static_ip_gateway(ifname, STATIC_IPV6);
                if (err == NM_ERR_VALUE_NOT_FOUND)
                                        err = 0;
...
uint32_t nm_get_ipv6_gateway(const char *ifname, char **ppszIPv6Gateway) {
        char *pszIPv6Gateway = NULL, **ppszGwAddrList = NULL;
        uint32_t err = 0;
        size_t count = 0;

        if (!IS_VALID_INTERFACE_NAME(ifname) || !ppszIPv6Gateway) {
                        err = NM_ERR_INVALID_PARAMETER;
                        bail_on_error(err);
        }

        err = nm_get_ip_default_gateway(ifname, DHCP_IPV6, &pszIPv6Gateway);
        if (err == NM_ERR_VALUE_NOT_FOUND)
                        err = 0;
        ...
        if (pszIPv6Gateway == NULL) {
                        err = nm_get_static_ip_gateway(ifname, STATIC_IPV6, &count, &ppszGwAddrList);
                        bail_on_error(err);
 ...
 */
static uint32_t nm_add_route_section(NET_IP_ROUTE *pRoute) {
        uint32_t err = 0, dwNumSections = 0, i;
        char *file_name = NULL, buf[MAX_LINE] = {0};
        PCONFIG_INI pConfig = NULL;
        PSECTION_INI *ppSections = NULL, pSection = NULL;
        PKEYVALUE_INI pDestKeyVal = NULL;

        if (!pRoute || !IS_VALID_INTERFACE_NAME(pRoute->ifname) || IS_NULL_OR_EMPTY(pRoute->pszDestNetwork) ||
            IS_NULL_OR_EMPTY(pRoute->pszGateway)) {
                        err = NM_ERR_INVALID_PARAMETER;
                        bail_on_error(err);
        }

        err = nm_get_network_conf_filename_for_update(pRoute->ifname, &file_name);
        bail_on_error(err);

        err = ini_cfg_read(file_name, &pConfig);
        bail_on_error(err);

        err = ini_cfg_find_sections(pConfig, SECTION_ROUTE, &ppSections, &dwNumSections);
        bail_on_error(err);

        for (i = 0; i < dwNumSections; i++) {
                pDestKeyVal = ini_cfg_find_key_value(ppSections[i], KEY_DEST, pRoute->pszDestNetwork);
                if (pDestKeyVal != NULL) {
        ...
        err = ini_cfg_save(file_name, pConfig);
        bail_on_error(err);
...
static uint32_t nm_delete_route_section(NET_IP_ROUTE *pRoute) {
        uint32_t err = 0, dwNumSections = 0, i;
        char *file_name = NULL;
        PCONFIG_INI pConfig = NULL;
...
static uint32_t nm_get_routes(const char *ifname, size_t *pCount, NET_IP_ROUTE ***pppRoutes) {
        uint32_t err = 0, dwNumSections = 0, i;
        char *file_name = NULL;
        PCONFIG_INI pConfig = NULL;
        PSECTION_INI *ppSections = NULL;
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pCount || !pppRoutes) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        for (i = 0; i < dwNumSections; i++) {
                        ...
                        err = netmgr_alloc_string(ifname, &ppRoutes[i]->ifname);
                        bail_on_error(err);
...
uint32_t nm_add_static_ip_route(NET_IP_ROUTE *pRoute) {
        uint32_t err = 0;
        uint8_t prefix = 255;
        ...
        if (!IS_VALID_INTERFACE_NAME(pRoute->ifname) ||
...
uint32_t nm_delete_static_ip_route(NET_IP_ROUTE *pRoute) {
        uint32_t err = 0;
        uint8_t prefix = 255;
        char szDestAddr[INET6_ADDRSTRLEN+5];
        char szbufDestAddr[INET6_ADDRSTRLEN+128];
...
uint32_t nm_get_static_ip_routes(const char *ifname, size_t *pCount, NET_IP_ROUTE ***pppRoutesList) {
        uint32_t err = 0;
        ...
        /* TODO: If ifname == NULL, get static route for all if */

        err = nm_get_routes(ifname, pCount, pppRoutesList);
        bail_on_error(err);
...
static uint32_t nm_space_delimited_string_append(size_t count, const char **ppszStrings, const char *pszCurrentString, char **ppszNewString) {
        uint32_t err = 0;
        size_t i, bytes = 0;
...
static uint32_t nm_space_delimited_string_to_list(const char *pszString, size_t *pCount, char ***pppszStringList) {
        uint32_t err = 0;
        size_t i = 0, count = 0;
...
static uint32_t nm_get_dns_mode(const char *ifname, NET_DNS_MODE *pMode) {
        uint32_t err = 0;
        NET_DNS_MODE mode;
        char *file_name = NULL;
        ...
        err = nm_get_key_value(file_name, SECTION_DHCP, KEY_USE_DNS,
                               &pszUseDnsValue);
...
uint32_t nm_add_dns_server(const char *ifname, const char *pszDnsServer) {
        uint32_t err = 0;
        NET_DNS_MODE mode;
        char *file_name = NULL;
        ...
        if (IS_NULL_OR_EMPTY(pszDnsServer) || !(is_ipv4_addr(pszDnsServer) || is_ipv6_addr(pszDnsServer))) {
                        err = NM_ERR_INVALID_PARAMETER;
                        bail_on_error(err);
                }
        ...
        if (ifname != NULL) {
                        ...
                        err = nm_get_resolved_conf_filename(&file_name);
                        sprintf(szSectionName, SECTION_RESOLVE);
        }

        bail_on_error(err);

        err = nm_get_key_value(file_name, szSectionName, KEY_DNS,
        ...
        err = nm_space_delimited_string_append(1, &pszDnsServer, pszCurrentDnsServers, &pszNewDnsServersValue);
        bail_on_error(err);

        err = nm_set_key_value(file_name, szSectionName, KEY_DNS,  pszNewDnsServersValue, 0);
        bail_on_error(err);
...
uint32_t nm_delete_dns_server(const char *ifname,const char *pszDnsServer) {
        uint32_t err = 0;
        NET_DNS_MODE mode;
        char *file_name = NULL;
        char szSectionName[MAX_LINE];
        ...
        pszNewDnsServersValue = (strlen(pszCurrentDnsServers) > 0) ? pszCurrentDnsServers : NULL;
...
uint32_t nm_set_dns_servers(const char *ifname, NET_DNS_MODE mode, size_t count, const char **ppszDnsServers) {
        uint32_t err = 0;
        char *file_name = NULL;
        ...
        err = nm_space_delimited_string_append(count, ppszDnsServers, NULL, &pszDnsServersValue);
        bail_on_error(err);
        ...
        if (mode == DHCP_DNS) {
                        ...
                        if (count == 0)
                                err = nm_set_key_value(file_name, szSectionName, KEY_DNS, NULL, 0);
                ...
                err = nm_set_key_value(file_name, szSectionName, KEY_DNS, pszDnsServersValue, 0);
...
uint32_t nm_get_dns_servers(const char *ifname, NET_DNS_MODE *pMode, size_t *pCount, char ***pppszDnsServers) {
        uint32_t err = 0;
        char *file_name = NULL, *pszFileBuf = NULL;
        ...
        if (ifname == NULL) {
                ...
                err = nm_get_key_value(file_name, szSectionName, KEY_DNS, &pszDnsServersValue);
                        if (err == NM_ERR_VALUE_NOT_FOUND) {
                                        err = 0;
                                        goto error;
                                }
                        ...
                        err = nm_space_delimited_string_to_list(pszDnsServersValue, &count, &ppszDnsServersList);
                        bail_on_error(err);
                }
...
uint32_t nm_add_dns_domain(const char *ifname, const char *pszDnsDomain) {
        uint32_t err = 0, sdVersion = 0;
        char *file_name = NULL;
        char szSectionName[MAX_LINE];
        ...
        err = nm_get_key_value(file_name, szSectionName, KEY_DOMAINS, &pszCurrentDnsDomains);
        if (err != NM_ERR_VALUE_NOT_FOUND)
                        bail_on_error(err);


        err = nm_space_delimited_string_append(1, &pszDnsDomain, pszCurrentDnsDomains, &pszDnsDomainsValue);
        bail_on_error(err);

        err = nm_set_key_value(file_name, szSectionName, KEY_DOMAINS, pszDnsDomainsValue, 0);
        bail_on_error(err);
...
uint32_t nm_delete_dns_domain(const char *ifname, const char *pszDnsDomain) {
        uint32_t err = 0, sdVersion = 0;
        char *file_name = NULL;
        char szSectionName[MAX_LINE];
        char *pszCurrentDnsDomains = NULL;
        ...
        pszNewDnsDomainsList = (strlen(pszCurrentDnsDomains) > 0) ? pszCurrentDnsDomains : NULL;

        err = nm_set_key_value(file_name, szSectionName, KEY_DOMAINS, pszNewDnsDomainsList, 0);
        bail_on_error(err);
...
uint32_t nm_set_dns_domains(const char *ifname, size_t count, const char **ppszDnsDomains) {
        uint32_t err = 0, sdVersion = 0;
        char *file_name = NULL;
        char szSectionName[MAX_LINE];
        char *pszDnsDomainsValue = NULL;
        ...
        err = nm_space_delimited_string_append(count, ppszDnsDomains, NULL, &pszDnsDomainsValue);
        bail_on_error(err);

        if (count == 0)
                err = nm_set_key_value(file_name, szSectionName, KEY_DOMAINS, NULL, 0);
        else
                err = nm_set_key_value(file_name, szSectionName, KEY_DOMAINS, pszDnsDomainsValue, 0);
...
uint32_t nm_get_dns_domains(const char *ifname, size_t *pCount, char ***pppszDnsDomains) {
        uint32_t err = 0;
        size_t count = 0;
        char szSectionName[MAX_LINE];
        char *file_name = NULL, *pszFileBuf = NULL;
        ...
        if (ifname == NULL) {
                        ...
                        err = nm_space_delimited_string_to_list(pszDnsDomainsValue, &count, &ppszDnsDomainsList);
                ...
                err = nm_get_key_value(file_name, szSectionName, KEY_DOMAINS, &pszDnsDomainsValue);
                        if (err == NM_ERR_VALUE_NOT_FOUND) {
                                        err = 0;
                                        goto error;
                                }
 ...
 */
uint32_t nm_set_iaid(const char *ifname, uint32_t iaid) {
        uint32_t err = 0;
        char *file_name = NULL;
        char szValue[MAX_LINE] = "";
        int lockId;
        ...
        if (iaid > 0)
                err = nm_set_key_value(file_name, SECTION_DHCP, KEY_IAID, szValue, 0);
        else
                err = nm_set_key_value(file_name, SECTION_DHCP, KEY_IAID, NULL, 0);
...
uint32_t nm_get_iaid(const char *ifname, uint32_t *pIaid) {
        uint32_t err = 0;
        char *file_name = NULL;
        char *pszIaid = NULL;

        if (!IS_VALID_INTERFACE_NAME(ifname) || !pIaid) {
                        err = NM_ERR_INVALID_PARAMETER;
                        bail_on_error(err);
                }
        ...
        err = nm_get_key_value(file_name, SECTION_DHCP, KEY_IAID, &pszIaid);
        bail_on_error(err);
...
uint32_t nm_set_duid(const char *ifname, const char *pszDuid) {
        uint32_t err = 0;
        char *file_name = NULL;
        const char *duidType;
        uint16_t n1, n2;
        ...
        if (ifname != NULL)
                ...
                err = nm_get_networkd_conf_filename(&file_name);

        bail_on_error(err);

        if ((pszDuid == NULL) || (strlen(pszDuid) == 0)) {
                err = nm_set_key_value(file_name, SECTION_DHCP, KEY_DUID_TYPE,
                                       NULL, F_CREATE_CFG_FILE);
                bail_on_error(err);

                err = nm_set_key_value(file_name, SECTION_DHCP, KEY_DUID_RAWDATA,
                                       NULL, F_CREATE_CFG_FILE);
                bail_on_error(err);
        } else {
                ...
                err = nm_set_key_value(file_name, SECTION_DHCP, KEY_DUID_TYPE, duidType, F_CREATE_CFG_FILE);
                bail_on_error(err);

                err = nm_set_key_value(file_name, SECTION_DHCP, KEY_DUID_RAWDATA, szDuid, F_CREATE_CFG_FILE);
                bail_on_error(err);
...
uint32_t nm_get_duid(const char *ifname, char **ppszDuid) {
        uint32_t err = 0;
        char *file_name = NULL;
        uint16_t duidType;
        ...
        err = nm_get_key_value(file_name, SECTION_DHCP, KEY_DUID_TYPE, &pszDuidType);
        bail_on_error(err);
        ...
        err = nm_get_key_value(file_name, SECTION_DHCP, KEY_DUID_RAWDATA, &pszDuid);
        bail_on_error(err);
...
uint32_t nm_set_ntp_servers(size_t count, const char **ppszNtpServers) {
        uint32_t err = 0;
        char *pszBuf = NULL;
        size_t i;
        int lockId;
...
uint32_t nm_add_ntp_servers(size_t count, const char **ppszNtpServers) {
        uint32_t err = 0;
        size_t i;
        char *pszBuf = NULL;
        int lockId;
...
uint32_t nm_delete_ntp_servers(size_t count, const char **ppszNtpServers) {
        uint32_t err = 0;
        size_t i;
        char *pszBuf = NULL;
        int lockId;
...
uint32_t nm_get_ntp_servers(size_t *pCount, char ***pppszNtpServers) {
        uint32_t err = 0;
        ...
        if (count > 0) {
                err = netmgr_alloc((count * sizeof(char *)), (void **)&ppszNtpServersList);
                bail_on_error(err);
...
uint32_t nm_add_firewall_rule(NET_FW_RULE *pNetFwRule) {
        uint32_t err = 0;
        ...
        err = netmgr_alloc_string_printf(&pszFwRule, "iptables %s", pNetFwRule->pszRawFwRule);
        bail_on_error(err);
        ...
        err = netmgr_alloc(strlen(pszFileBuf) + strlen(pszFwRule) + 4, (void **)&pszNewFileBuf);
        bail_on_error(err);
...
uint32_t nm_delete_firewall_rule(NET_FW_RULE *pNetFwRule) {
        uint32_t err = 0;
        size_t i, lineCount = 0;
        char *pszFileBuf = NULL, *pszFwRule = NULL, **ppszLineBuf = NULL;
        ...
        for (i = 0; i < lineCount; i++) {
                if ((!strcmp(pszFwRule, "iptables *") && ((p = strstr(ppszLineBuf[i], "iptables")) == ppszLineBuf[i])) ||
...
uint32_t nm_get_firewall_rules(size_t *pCount, NET_FW_RULE ***pppNetFwRules) {
        uint32_t err = 0;
        ...
        for (i = 0; i < lineCount; i++) {
                ...
                err = netmgr_alloc(ruleCount * sizeof(NET_FW_RULE *), (void **)&ppNetFwRules);
                bail_on_error(err);

                for (i = 0; i < lineCount; i++) {
                        p = strstr(ppszLineBuf[i], "iptables");
                        if ((p != NULL) && (p == ppszLineBuf[i])) {
                                err = netmgr_alloc(sizeof(NET_FW_RULE), (void **)&ppNetFwRules[j]);
                                bail_on_error(err);

                                ppNetFwRules[j]->type = FW_RAW;

                                err = netmgr_alloc_string_len(ppszLineBuf[i], strlen(ppszLineBuf[i])-1, &ppNetFwRules[j]->pszRawFwRule);
                                bail_on_error(err);
...
/* Misc APIs */
uint32_t nm_set_hostname(const char *pszHostname){
...
uint32_t nm_get_hostname(char **ppszHostname) {
        sd_bus_error bus_error = SD_BUS_ERROR_NULL;
        sd_bus_message *reply = NULL;
...
uint32_t nm_wait_for_link_up(const char *ifname, uint32_t timeout) {
        uint32_t err = 0;
...
static uint32_t nm_validate_netlink_ipaddr(
                                           const char *ifname,
        ...
        if (!IS_VALID_INTERFACE_NAME(ifname) || !pIfAddrMsg || !ifAddrMsgLen || !addrType) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }

        pRouteAttr = IFA_RTA(pIfAddrMsg);
        for (;RTA_OK(pRouteAttr, ifAddrMsgLen); pRouteAttr = RTA_NEXT(pRouteAttr, ifAddrMsgLen)) {
                if (pRouteAttr->rta_type != IFA_LOCAL && pRouteAttr->rta_type != IFA_ADDRESS)
                        continue;
                nlAddrType = 0;

                if (inet_ntop(pIfAddrMsg->ifa_family, RTA_DATA(pRouteAttr), szIpAddr, sizeof(szIpAddr)) == NULL)
                        err = errno;

                bail_on_error(err);


                err = netmgr_alloc_string_printf(&pszIpAddrPrefix, "%s/%u", szIpAddr, pIfAddrMsg->ifa_prefixlen);
                bail_on_error(err);

                err = nm_get_ip_addr_type(ifname, pszIpAddrPrefix, &nlAddrType);
                bail_on_error(err);
...
uint32_t nm_wait_for_ip(const char *ifname, uint32_t timeout, NET_ADDR_TYPE addrTypes) {
        ...
        err = nm_get_interface_ipaddr(ifname, addrTypes, &ipCount,&ppszIpAddrList);
        if (err == NM_ERR_VALUE_NOT_FOUND)
                        err = 0;
        ...
        for (i = 0; i < ipCount; i++) {
                        err = nm_get_ip_addr_type(ifname, ppszIpAddrList[i], &ipAddrType);
        ...
        while (1) {
                FD_ZERO(&readFd);
                        ...
                        while (pCurNetLinkMsgList) {
                                        ...
                                        err = nm_validate_netlink_ipaddr(ifname, pIfAddr, ifAddrMsgLen, addrTypes);
                                        if (err == 0){
                                                        ipAddrValid = 1;
                                                        break;
                                        }
...
uint32_t nm_set_network_param(const char *pszObjectName, const char *pszParamName, const char *pszParamValue ) {
        ...
        if (stat(pszObjectName, &fs) != 0) {
                ...
                err = nm_get_network_conf_filename(pszObjectName, &file_name);
                bail_on_error(err);
        } else {
                /* Check if pszObjectName is a regular file */
                if (TEST_FLAG(fs.st_mode, S_IFREG))
                        err = netmgr_alloc_string(pszObjectName, &file_name);
                else
                        err = NM_ERR_INVALID_PARAMETER;
        ...
        switch (op) {
        case '+':
                err = nm_add_key_value(file_name, pszSectionName, pszKeyName, pszParamValue, 0);
                bail_on_error(err);
                break;

        case '-':
                err = nm_delete_key_value(file_name, pszSectionName, pszKeyName, pszParamValue, 0);
                bail_on_error(err);
                break;
        default:
                err = nm_set_key_value(file_name, pszSectionName, pszKeyName, pszParamValue, 0);
                bail_on_error(err);
        }
...
uint32_t nm_get_network_param(const char *pszObjectName, const char *pszParamName, char **ppszParamValue) {
        ...
        if (IS_NULL_OR_EMPTY(pszObjectName) || IS_NULL_OR_EMPTY(pszParamName) || !ppszParamValue) {
                err = NM_ERR_INVALID_PARAMETER;
                bail_on_error(err);
        }
        ...
        err = nm_get_key_value(file_name, pszSectionName, pszKeyName, &pszParamValue);
        bail_on_error(err);
...
uint32_t nm_stop_network_service() {
        sd_bus_error bus_error = SD_BUS_ERROR_NULL;
        sd_bus_message *reply = NULL;
        uint32_t err = 0;
        sd_bus *bus = NULL;
...
uint32_t nm_restart_network_service() {
        sd_bus_error bus_error = SD_BUS_ERROR_NULL;
        sd_bus_message *reply = NULL;
        uint32_t err = 0;
        sd_bus *bus = NULL;
...
uint32_t nm_restart_dns_service() {
        sd_bus_error bus_error = SD_BUS_ERROR_NULL;
        sd_bus_message *reply = NULL;
        uint32_t err = 0;
        sd_bus *bus = NULL;
...
uint32_t nm_restart_ntp_service() {
        sd_bus_error bus_error = SD_BUS_ERROR_NULL;
        sd_bus_message *reply = NULL;
        uint32_t err = 0;
        sd_bus *bus = NULL;
...
uint32_t nm_reload_firewall_config() {
