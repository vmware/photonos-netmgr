...
)
{
    ...
    size_t count = 0;
    int family = 0, i = 0;
    const char *ntopRet = NULL;
    struct ifaddrs *ifaddr = NULL, *ifa = NULL;
    char **ppszIpAddrList = NULL;
    // Adding 4 extra bytes to append prefix.1 byte for / and 3 bytes for Prefix
    char szIpAddr[INET6_ADDRSTRLEN + 4];
    ...
    for (ifa = ifaddr; count && (ifa != NULL); ifa = ifa->ifa_next)
    {
        ...
        if (!strcmp(ifa->ifa_name, pszInterfaceName))
        {
            ...
            if (((family == AF_INET) && ip4Type) ||
                ((family == AF_INET6) && ip6Type))
            {
                ntopRet = inet_ntop(family, ifa->ifa_addr, szIpAddr, INET6_ADDRSTRLEN);
                if (ntopRet == NULL)
                {
                    err =  errno;
                    bail_on_error(err);
                ...
                err = get_ipaddr_prefix(ifa->ifa_netmask, &numBitsSet);
                bail_on_error(err);

                sprintf(szIpAddr,"%s/%hu", szIpAddr, numBitsSet);
    ...
    )
{
    ...
    //Command has 4 spaces, additional command option -I of size 2 and 1
    //additional byte for NULL at the end. 4+2+1= 7 extra space.
...
)
{
    ...
    if ((count != 0) && ip4Count &&
        (strcmp(ppszIpAddrList[0], ppszIpv4Addr[0]) != 0))
    {
        err = EINVAL;
        bail_on_error(err);
    }

    if ((count != 0) && !ip4Count && !ip6Count)
    {
