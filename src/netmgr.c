...
)
{
    ...
    if (TEST_FLAG(addrType, STATIC_IPV6)|| TEST_FLAG(addrType, DHCP_IPV6) ||
        TEST_FLAG(addrType, AUTO_IPV6) || TEST_FLAG(addrType, LINK_LOCAL_IPV6))
    {
...
#define BUFSIZE 8192
    ...
    if (IS_NULL_OR_EMPTY(pszInterfaceName) || (sockFd == -1) || !done)
    {
        err = EINVAL;
        bail_on_error(err);
    }
        ...
        for (; NLMSG_OK(nlHdr, readLen); nlHdr = NLMSG_NEXT(nlHdr, readLen))
        {
            ...
            switch (nlHdr->nlmsg_type)
            {
                case RTM_NEWADDR:
                    ...
                    if ((strcmp(pszInterfaceName, ifName) != 0) || !addrType)
                    {
                        break;
                    }
                    ...
                    err = nm_handle_nic_newaddr(pszInterfaceName,
                                                pIfAddr,
                                                ifAddrMsgLen,
                                                addrType,
                                                done);
                ...
                case RTM_NEWLINK:
                    ...
                    if (!strcmp(pszInterfaceName, ifName) && !addrType &&
...
)
{
    ...
    err = nm_open_netlink_socket(0, &sockFd);
    bail_on_error(err);
    ...
    while (1)
    {
        ...
        {
            err = nm_handle_netlink_event(pszInterfaceName, sockFd, 0, &done);
        }
        else
        {
            err = ETIME;
...
)
{
    ...
    err = nm_open_netlink_socket(addrTypes, &sockFd);
    bail_on_error(err);
    ...
    while (1)
    {
        ...
        {
            err = nm_handle_netlink_event(pszInterfaceName,
                                          sockFd,
                                          addrTypes,
                                          &done);
        }
